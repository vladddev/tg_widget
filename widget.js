var tgwidget_tl = function() {
    'use strict';
    const self = this;
    const CONFIG = {
        APPKEY: "1234567890987654321"
        ,PATH: {
            ROOT_URL: 'https://example.ru'
            ,getFullUrl: function (urlPart) {return `${this.ROOT_URL}/${urlPart}/?key=${CONFIG.APPKEY}`}
            ,getAuthUrl: function (urlPart) { return `${this.getFullUrl(`${self.system()['subdomain']}/${self.get_accounts_current().account.id}/${urlPart}`)}`}
        }
    };
    ...
    this.callbacks = {
        ...
        dpSettings: function () {
            const rootFunc = () => {
                ...
                dataAccess.getAllChats((res)=>{
                    ...
                    const html = self.render({ ref: '/tmpl/controls/checkboxes_dropdown.twig'}, {
                        class_name: 'tgwidget-tlgnotify-widget__whomsend',
                        items: (()=>{
                            const items = [];
                            res.forEach((elm)=>items.push({id: elm['chat_id'], option: elm['chat_name'], is_checked: selected.includes(elm['chat_id'])}));
                            return items;
                        })()
                    });
        ...
        onAddAsSource: function (pipeline_id) { return true; },
    };
    return this;
};
